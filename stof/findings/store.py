...
class FindingDB:
    ...
    def _connect(self) -> sqlite3.Connection:
        ...
        # WAL + NORMAL sync: same reasoning as session_store.py's
        # identical pragmas -- shares this same data/stof.db file, so
        # readers (dashboard/report queries) no longer block behind a
        # scan's own writes, and each commit skips the extra fsync
        # default (rollback-journal) mode pays.
